...
| **Occlusion Culling** | 저해상도 깊이 맵 위에 밉 피라미드(HZB)를 쌓아 사각형 영역의 최대 깊이를 적응적으로 샘플링하는 **CPU HZB** 방식. BVH와 이전 프레임 히스토리를 함께 사용. NDC Z는 원근 나눗셈 때문에 비선형이라 버리고 **뷰 공간 Z를 [0..1]로 선형화**해 판정. 오클루더를 바운딩 박스 그대로 쓰면 실제 실루엣보다 커서 잘못된 가림이 생기므로, 화면 사각형을 **중심 기준 50%로 축소**하고 깊이를 **박스의 먼 면(MaxZ)으로 기록**해 보수적으로 판정. 그리드 해상도는 **화면의 1/4 → 1/2로 상향**. 두 차례 롤백 후 재구현 |
