...
| **Occlusion Culling** | 저해상도 깊이 맵 위에 밉 피라미드(HZB)를 쌓아 사각형 영역의 최대 깊이를 적응적으로 샘플링하는 **CPU HZB** 방식. BVH와 이전 프레임 히스토리를 함께 사용. NDC Z는 원근 나눗셈 때문에 비선형이라 버리고 **뷰 공간 Z를 [0..1]로 선형화**해 판정, 초기 그리드 해상도를 절반으로 낮춰 정확도 개선. 두 차례 롤백 후 재구현 |
